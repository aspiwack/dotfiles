...
#+begin_src emacs-lisp
    ;; This lets Flycheck see Eglot's error. Otherwise only Flymake works.
    ;; It doesn't make a ton of difference, I'm just used to Flycheck
    ;; these days.
...
**** TODO Eglot configuration


***** TODO setup a keymap with
- ~eldoc-doc-buffer~ (display full documentation)
- ~eglot-format~ (format selection or buffer)
- ~eglot-rename~ (rename symbol)
- ~eglot-code-actions~ (list and pick code actions)
