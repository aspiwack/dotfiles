...
#+begin_src emacs-lisp
  ...
  (set-face-attribute 'aspiwack/programming nil :family "DejaVu Sans" :inherit 'default :height 1.15)
#+end_src
...
#+BEGIN_SRC emacs-lisp
  (use-package boon
    ...
    (define-key boon-command-map "_" 'undo-tree-redo)
...
C projects).
#+BEGIN_SRC emacs-lisp
;; It seems that the projectile configuration has a bit of a special status
;; (maybe, I can't say I've tested it thoroughly, just ran into issues) so using
;; a more Doom-esque configuration block.
(map!
 :after boon
...
The [[https://rocq-prover.org/][Rocq prover]], proof assistant and programming language, with [[https://github.com/ProofGeneral/PG][Proof general]].
